...
Fixed a bug on the autogeneration of the model: the model was not being autognerated by the run_vcs script, infact if you change the instructions configuration in config.json, only the parser was runned, and parsing correctly the instructions, but these instructions were not passed to the decoder_model script, and the model was never autogenerated. A new template command has been added in fmt.py and run_vcs ( autogen_cmd ).