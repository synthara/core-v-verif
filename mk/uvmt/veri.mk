...
###############################################################################
#
# Veri specific Makefile for the Core-V-Verif "uvmt" testbench.
# Verilator is a opensource
#
###############################################################################
...
UVM_HOME               ?= $(abspath $(shell readlink -f $(CORE_V_VERIF)/tools/uvm-verilator/))
USES_DPI = 1
...
ifeq ($(USES_DPI),1)
  ...
  DPILIB_VERILATOR_OPT = +define+UVM_NO_DPI
  DPILIB_TARGET =
endif
...
# VLOG (Compilation)
VERILATOR_FLAGS    ?= \
                    --unroll-count 256 --timing                                                                  \
                    -Wall --relative-includes                                                                    \
                    -Werror-PINMISSING                                                                           \
                    -Werror-IMPLICIT                                                                             \
                    -Wno-fatal                                                                                   \
                    -Wno-PINCONNECTEMPTY                                                                         \
...
###############################################################################
# VSIM (Simulaion)
SIM_FLAGS        += $(VSIM_USER_FLAGS)
SIM_FLAGS        += $(USER_RUN_FLAGS)
SIM_FLAGS        += -sv_seed $(RNDSEED)
SIM_FLAGS        += -64
SIM_FLAGS        += -suppress 7031
SIM_FLAGS        += -suppress 8858
SIM_FLAGS        += -suppress 8522
SIM_FLAGS        += -suppress 8550
SIM_FLAGS        += -suppress 8549
SIM_FLAGS        += -permit_unmatched_virtual_intf
SIM_DEBUG_FLAGS  ?= -debugdb
SIM_GUI_FLAGS    ?= -gui -debugdb
SIM_SCRIPT_DIR	  = $(abspath $(MAKE_PATH)/../tools/vsim)

VERILATOR_UVM_ARGS      = +incdir+$(UVM_HOME)/src $(UVM_HOME)/src/uvm_pkg.sv

VERILATOR_FLAGS += $(VERILATOR_UVM_ARGS)

SIM_FLAGS += -sv_lib $(basename $(OVP_MODEL_DPI))
ifeq ($(call IS_YES,$(USE_ISS)),YES)
SIM_FLAGS += +USE_ISS
else
SIM_FLAGS += +DISABLE_OVPSIM
endif
ifeq ($(call IS_YES,$(TEST_DISABLE_ALL_CSR_CHECKS)),YES)
SIM_FLAGS +="+DISABLE_ALL_CSR_CHECKS"
endif
ifneq ($(TEST_DISABLE_CSR_CHECK),)
SIM_FLAGS += +DISABLE_CSR_CHECK=$(TEST_DISABLE_CSR_CHECK)
endif

SIM_FLAGS += -sv_lib $(basename $(DPI_DASM_LIB))
SIM_FLAGS += -sv_lib $(basename $(abspath $(SVLIB_LIB)))
...
VERILATOR_FLAGS  += $(VOPT_COV)
SIM_FLAGS  += $(VSIM_COV)
#SIM_FLAGS  += -do 'set TEST ${VSIM_TEST}; source $(VSIM_SCRIPT_DIR)/cov.tcl'
endif
...
SIM_FLAGS += $(VSIM_WAVES_ADV_DEBUG)
else
SIM_FLAGS += --trace --trace-fst --trace-params --trace-structs
endif
endif

ifeq ($(call IS_YES,$(ADV_DEBUG)),YES)
VERILATOR_FLAGS += $(VOPT_WAVES_ADV_DEBUG)
endif
...
no_rule:
	...
	@echo 'try "make SIMULATOR=veri sanity" (or just "make sanity" if shell ENV variable SIMULATOR is already set).'
...
comp:
	@echo "$(BANNER)"
	@echo "* Running verilator in $(SIM_CFG_RESULTS)"
...
run: $(VSIM_RUN_PREREQ)
	@echo "$(BANNER)"
	@echo "* Running verilator in $(RUN_DIR)"
	@echo "* Log: $(RUN_DIR)/verilator-$(VSIM_TEST).log"
	@echo "$(BANNER)"
	mkdir -p $(RUN_DIR) && \
	cd $(RUN_DIR) && \
		$(VSIM) \
...
test: SIM_FLAGS += +firmware=$(SIM_TEST_PROGRAM_RESULTS)/$(TEST_PROGRAM)$(OPT_RUN_INDEX_SUFFIX).hex
test: SIM_FLAGS += +elf_file=$(SIM_TEST_PROGRAM_RESULTS)/$(TEST_PROGRAM)$(OPT_RUN_INDEX_SUFFIX).elf
test: SIM_FLAGS += +itb_file=$(SIM_TEST_PROGRAM_RESULTS)/$(TEST_PROGRAM)$(OPT_RUN_INDEX_SUFFIX).itb
test: hex run
